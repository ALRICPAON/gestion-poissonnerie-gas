...
function getCaForEan(artEanRaw, ventesEANNet) {
  ...
  for (const k of keys) {
    ...
    if (minLen > 3 && k.slice(-minLen) === rawStr.slice(-minLen)) return Number(ventesEANNet[k] || 0);
    if (normalized && k.endsWith(normalized.slice(-minLen))) return Number(ventesEANNet[k] || 0);
  }

  return 0;
}

/* ---------- Expand plateaux from CA ---------- */
...
/**
 * Recompute stock_articles 'PLU_xxx' poids from lots (only open lots)
 */
...
async function createAddLotAndMovement(plu, qty, unitCost, sessionId, opts = {}) {
  ...
  const lotObj = {
    ...
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp(),
    origin: opts.origin || "inventaire_session",
    sessionId,
    closed: false
  };
